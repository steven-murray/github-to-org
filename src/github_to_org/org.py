...
def get_node_level(line) -> int:
    """Get the node level of a line, returning zero if the line doesn't define a node."""
...
def get_org_node(repo_name: str, fname: [str, Path]) -> Tuple[str, int]:
    ...
    with open(fname, 'r') as fl:
        ...
        for i, line in enumerate(lines):
            ...
            if node_level and repo_name.split('/')[1].lower() in line.lower() and ':gh:' in line:
                in_node = True
                start = i
    ...
    for line in node.split('\n'):
        if f'{repo.lower()}/issues/' in line.lower():
            ...
            try:
                ...
                warnings.warn(f"This line has formatting incompatible with gh2org: {line}")
...
def convert_issue_list_to_org(issues: List[Issue], existing_tasks: List[int], settings: dict, level: int) -> str:
...
def issue_to_node_str(issue: Issue, priority: [None, str], schedule: dt.datetime, level: int):
    if priority is None:
        priority = ''
    else:
        priority = f'[#{priority}]'
    ...
    return f"{'*'*level} TODO {priority} [[{issue.html_url}][{issue.repository.name}/{issue.number}]]: {issue.title}\n"
